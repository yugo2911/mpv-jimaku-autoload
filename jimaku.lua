...
download_selected_subtitle_action = function(file)
    ...
    if download_result.status == 0 then
        -- Integrated: Check if it's an archive and handle it
        local ext = subtitle_path:match("%.([^%.]+)$")
        if ext then ext = ext:lower() end
        
        if ext == "zip" or ext == "rar" or ext == "7z" or ext == "tar" or ext == "gz" then
            -- It's an archive - use the integrated archive handling
            if handle_archive_file_integrated(subtitle_path, "select", file) then
                mp.osd_message("✓ Extracted and loaded archive", 3)
            else
                mp.osd_message("Failed to extract archive", 3)
            end
        else
            -- Regular subtitle file
            mp.commandv("sub-add", subtitle_path, "select")
            mp.osd_message("✓ Subtitle loaded", 3)
        end
...
-------------------------------------------------------------------------------
-- INTEGRATED ARCHIVE HANDLING FUNCTION
-- Combines archive detection, extraction, and smart filtering
-------------------------------------------------------------------------------

-- Integrated archive handling that works seamlessly with download_subtitle_smart
handle_archive_file_integrated = function(archive_path, load_flag, file_info)
    debug_log("Integrated archive handling: " .. archive_path)
    ...
    local target_season = menu_state.current_match and menu_state.current_match.season or
                         ...
                         1
    local anilist_id = menu_state.current_match and menu_state.current_match.anilist_id
    
    debug_log(string.format("Archive filtering context: Title='%s', Season=%s, Episode=%s, AniList ID=%s", 
        target_title, target_season or "nil", target_episode, anilist_id or "nil"))
    ...
    local subtitle_files = scan_for_subtitles(extract_dir, extract_dir, target_title, target_episode, target_season, 3, anilist_id)
    ...
    for i = 1, files_to_load do
        ...
        local flag = (i == 1) and load_flag or "auto"
        
        debug_log(string.format("Loading [%d/%d]: %s (flag: %s)", 
            i, files_to_load, sub_info.name, flag))
        ...
        if success then
            ...
            debug_log("Successfully loaded: " .. sub_info.name)
        else
            debug_log("Failed to load: " .. sub_info.name, true)
        end
    end
...
-- Smart subtitle download with integrated archive handling
download_subtitle_smart = function(entry_id, target_episode, target_season, seasons_data, anilist_entry, is_auto)
    ...
    for i = 1, max_downloads do
        ...
        if download_result.status == 0 then
            ...
            if ext == "zip" or ext == "rar" or ext == "7z" or ext == "tar" or ext == "gz" then
                -- Use integrated archive handling
                local load_flag = (success_count == 0) and "select" or "auto"
                if handle_archive_file_integrated(subtitle_path, load_flag, subtitle_file) then
...
-------------------------------------------------------------------------------
-- ARCHIVE HELPER FUNCTIONS (Integrated)
-------------------------------------------------------------------------------
...
-- Recursive scan for subtitles with filtering
local function scan_for_subtitles(dir_path, base_dir, target_title, target_episode, target_season, max_depth, target_anilist_id)
    ...
    -- Read metadata for current directory
    ...
    if current_kitsu then
        ...
        elseif target_title and current_kitsu.name then
            -- Simple title matching for metadata
            if current_kitsu.name:lower():find(target_title:lower(), 1, true) then
                dataset_matches_target = true
            end
        end
    end
        ...
        for _, item in ipairs(files) do
            ...
            if ext then
                ...
                if ext == "ass" or ext == "srt" or ext == "vtt" or ext == "sub" then
                    -- Check if file matches our search
                    local filename_lower = item:lower()
                    local title_lower = target_title:lower()
                    
                    -- Simple matching logic
                    local matches = false
                    if filename_lower:find(title_lower, 1, true) then
                        matches = true
                    elseif dataset_matches_target then
                        matches = true
                    end
...
local function try_extract_archive(archive_path, extract_dir)
    ...
    -- package.config, os.execute, escape_path
    if STANDALONE_MODE then
        if is_windows then
            os.execute('if not exist "' .. extract_dir:gsub('/', '\\') .. '" mkdir "' .. extract_dir:gsub('/', '\\') .. '"')
        else
        ...
        if is_windows then
            mp.command_native({
                ...
                playback_only = false,
                capture_stdout = true,
                args = {"cmd", "/c", "if", "not", "exist", extract_dir:gsub('/', '\\'), "mkdir", extract_dir:gsub('/', '\\')}
            })
        else
            mp.command_native({
                name = "subprocess",
...
-------------------------------------------------------------------------------
local search_local_subtitle_cache = function(parsed, is_auto, anilist_id)
    if not (parsed and parsed.title) then return false end

    local target_episode = tonumber(parsed.episode) or 1
    local target_season = tonumber(parsed.season) or 1
    ...
    if not subtitle_files or #subtitle_files == 0 then return false end

    -- table.sort
    table.sort(subtitle_files, function(a, b)
        local a_ep = tonumber(a.episode)
        local b_ep = tonumber(b.episode)
        if a_ep and b_ep then
            if a_ep ~= b_ep then return a_ep < b_ep end
    ...
    -- mp.commandv, table.insert
    for i = 1, max_to_load do
        local sub = subtitle_files[i]
        if sub and sub.path then
            local flag = (i == 1) and "select" or "auto"
            local success = pcall(function() mp.commandv("sub-add", sub.path, flag) end)
            if success then
                loaded_count = loaded_count + 1
                if menu_state and menu_state.loaded_subs_files then
                    table.insert(menu_state.loaded_subs_files, sub.name or sub.path)
                end
            end
        end
    end

    -- string.format, conditional_osd
    if loaded_count > 0 then
        conditional_osd(string.format("✓ OFFLINE: Loaded %d subs", loaded_count), 5, is_auto)
        return true
    end
    return false
end
